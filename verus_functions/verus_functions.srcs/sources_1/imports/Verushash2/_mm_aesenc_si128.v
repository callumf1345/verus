//verilog imp of aesenc_si128 for verushash2

module aesenc (
input clk,
input [7:0] s,
input [7:0] rk, //a constant in c so we can write as a local param maybe?
output reg [7:0] s_out

);

//values were 0x1b etc, check if this is the right format, given ff = 11111111 it should be?
//remember this concats all the values so you need to address the right range of memory NOT array indexs when calling it 
...
//these are all unsigned char in c so surly 8 bits? But then some are indexed upto 16..?

reg [7:0] v[3:0][3:0];
reg [7:0] t, u; //being passed inside of replication, might throw a strop if so use an array indexed i to fix?

genvar i,j, k;
generate 
	for (i =0; i < 16; i = i +1) begin //check ordering with peter, ++i is used, stack seems to suggest operation is the same
			 always @(*) v[((i/4)+4 -(i%4))%4][i %4] = sbox[s[i]*8 +:7]; //genvar may be a waste of time, we know the value of i so we can just express the relationship fully surely?
	end
	//test above logic by sending just value of i to each v cell then comparing all v cell values to c printout 
	for(j = 0; j <4; j = j +1) begin //check ordering here too
			always @(posedge clk) 
				begin
				t = v[j][0];
				u = v[j][0] ^v[j][1]  ^ v[j][2] ^ v[j][3];
				v[j][0] = v[j][0] ^ (u ^ XT(v[j][0] ^ v[j][1]));
				v[j][1] = v[j][1] ^ (u ^ XT(v[j][1] ^ v[j][2]));
				v[j][2] = v[j][2] ^ (u ^ XT(v[j][2] ^ v[j][3]));
				v[j][3] = v[j][3] ^ (u ^ XT(v[j][3] ^ t));
				end 
	end 
	for (k =0; k <16; k = k +1) begin
		always @(posedge clk) s_out[k] = v[k / 4][k % 4] ^ rk[k]; //not sure how blocking assigns interact between different always blocks?
		end
	
endgenerate //might benefit from begin written for clock cycles? given v is dependent on s and s is dependent on v later on?
...
assign out = (((x) << 1) ^ ((((x) >> 7) & 1) * 8'h1b)); // end constant was 0x1b change if errors
